...
export default function ImageGallerySection(props) {
    const { elementId, colors, backgroundImage, badge, title, subtitle, images = [], motion, styles = {} } = props;
    ...
    return (
        ...
        >
            {images.map((image, index) => (
                <ImageBlock key={index} {...image} data-sb-field-path={`.${index}`} />
            ))}
        </div>
    );
}
        ...
        <div className="w-full">
            ...
            >
                <div className={classNames('sb-image-strip', 'overflow-hidden', 'whitespace-nowrap', 'relative')} style={imageGridStyle}>
                    ...
                    >
                        <div className="sb-image-strip-content flex justify-around" data-sb-field-path=".images">
                            {images.map((image, index) => (
                                <ImageBlock key={index} {...image} data-sb-field-path={`.${index}`} />
                            ))}
                        </div>
