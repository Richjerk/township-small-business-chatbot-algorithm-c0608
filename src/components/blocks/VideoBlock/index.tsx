...
function YouTubeVideo({ id, autoplay, loop, muted, controls = true }) {
    const paramsObj: any = {};
...
function VimeoVideo({ id, autoplay, loop, muted, controls = true }) {
    const paramsObj: any = {};
    paramsObj.autoplay = autoplay ? '1' : '0';
    paramsObj.controls = controls ? '1' : '0';
    paramsObj.loop = loop ? '1' : '0';
...
function SelfHostedVideo({ url, id, poster, autoplay, loop, muted, controls = true }) {
    return (
        <video
            ...
            className="absolute left-0 top-0 h-full w-full"
        >
            <source src={url} type={id} data-sb-field-path=".url#@src" />
        </video>
    );
}
